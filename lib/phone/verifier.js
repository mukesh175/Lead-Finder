...
// Reads a value from the first path that exists, so one reader copes with both
// the flat legacy shape and the grouped Phone Intelligence shape.
...
function readAbstract(payload) {
  ...
  return {
    ...
    lineType: pick(payload, ["carrier.line_type", "line_type", "type", "validation.line_type"]),
    carrier: pick(payload, ["carrier.name", "carrier"]),
    country: pick(payload, ["location.country.name", "country.name", "country_name", "country"]),
    location: pick(payload, ["location.city", "location.region", "location"]),
    formatted: pick(payload, [
