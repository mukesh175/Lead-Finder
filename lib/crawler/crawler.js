...
import { isPlatformUrl } from "./platforms";
...
export async function analyzeWebsite(entryUrl, { maxPages = limits.maxPagesPerLead, seedPages = null } = {}) {
  ...
  while (queue.length > 0 && pages.length < maxPages) {
    ...
    let parsed;
    try {
      // Platform pages are full of the platform's own ids and numbers, so only
      // explicitly declared contacts (tel: links, structured data) are trusted.
      parsed = parsePage(fetched.html, fetched.url, {
        scanBodyText: !isPlatformUrl(fetched.url),
      });
    } catch {
      continue;
    }
