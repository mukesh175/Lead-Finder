...
import { phoneDigits } from "../crawler/parse";
import { isPlatformUrl, isGenericTitle, hostOf } from "../crawler/platforms";
...
export async function startSearch(user, { keyword, location, limit, mode = "business", sources = [] }) {
  ...
  const activeSources = isIntent ? resolveSources(sources) : [];
  ...
  try {
    const results = await runProviderSearch({
      keyword,
      location,
      limit: resultLimit,
      // Intent mode rewrites the query towards people asking for the service
      // and restricts it to the chosen public sources.
      ...(isIntent
        ? { query: intentQuery(keyword, location), includeDomains: domainsFor(activeSources) }
        : {}),
    });
    if (results.length === 0) {
...
async function ingestResult(userId, record, result) {
  const isIntent = record.mode === "intent";
  const website = result.url;
  const domain = rootDomain(website);

  const analysis = await analyzeWebsite(website, {
    // An intent post is a single public page - never crawl the host platform.
    maxPages: isIntent ? 1 : limits.maxPagesPerLead,
  ...
  const onPlatform = isPlatformUrl(website);
  const companyName = pickCompanyName({
    siteName: onPlatform ? null : pages.map((p) => p.siteName).find(Boolean),
  ...
  const intentQuote = isIntent
    ? extractIntentQuote(pages[0]?.description || result.snippet) ||
      extractIntentQuote(result.title)
    : null;

  const draft = {
    name: null,
    // An intent lead is a person's public post, not a company website, so the
    // post is kept only as the source and no company is invented for it.
    companyName: isIntent ? null : companyName,
    website: isIntent ? null : website,
    websiteKey: isIntent ? null : normalizeWebsite(website),
    email: email || null,
    emailStatus,
    phone: phones[0] || null,
    phoneDigits: phoneDigits(phones[0]),
    ...
    sourceType: isIntent
      ...
      : onPlatform
        ...
        : pages.length > 0
          ? "website_analysis"
          : "search_result",
    sourceGroup: isIntent ? groupForUrl(result.url) : null,
    intentQuote,
    description: result.title || pages[0]?.description || result.snippet || null,
    contactPageUrl: isIntent ? null : contactPage?.url || null,
  ...
  if (existing) {
    await prisma.leadSource.create({
      data: {
        ...
        method: sourceMethod(isIntent, pages.length),
        keyword: record.keyword,
      },
    });
...
function sourceMethod(isIntent, pageCount) {
  if (isIntent) return "public_post";
  return pageCount > 0 ? "public_website_page" : "search_result_metadata";
}

/**
 * A page title is only a company name when it actually names the business.
 * "Homepage", "Instagram" and "Google Docs" are the page or the platform, so
 * they fall back to the domain rather than being stored as the company.
 */
function pickCompanyName({ siteName, title, domain, onPlatform, url }) {
  if (siteName && !isGenericTitle(siteName)) return siteName.slice(0, 120);

  for (const part of String(title || "").split(/[|·•–—]/)) {
    // "Mani Sumit (@mani.sumit)" -> "Mani Sumit"
    const candidate = part.replace(/\(@[^)]+\)/g, "").trim();
    if (candidate && !isGenericTitle(candidate) && candidate.length <= 120) {
      return candidate;
    }
  }

  // On a platform the public handle is the only name available; the platform's
  // own domain would name nobody.
  if (onPlatform) return handleFromUrl(url);
  return domain || null;
}

/** instagram.com/kabragroup -> "kabragroup"; skips generic path prefixes. */
function handleFromUrl(url) {
  const segments = String(url || "")
    .split("?")[0]
    .split("/")
    .filter(Boolean)
    .slice(2); // drop protocol + host
  const skip = new Set(["in", "p", "post", "posts", "company", "pages", "watch", "channel", "user"]);
  const handle = segments.find((part) => !skip.has(part.toLowerCase()) && part.length > 1);
  return handle ? decodeURIComponent(handle).replace(/[-_.]+/g, " ").slice(0, 120) : null;
}
