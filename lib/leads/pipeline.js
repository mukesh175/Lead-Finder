...
import { search as runProviderSearch } from "../search/searchProvider";
...
export async function startSearch(user, { keyword, location, limit }) {
  ...
  const resultLimit = Math.min(limit, limits.maxResultsPerSearch);
  ...
  try {
    ...
    if (results.length === 0) {
      await prisma.search.update({
        where: { id: record.id },
        data: { status: "COMPLETED", resultsFound: 0, stats: emptyStats(), queue: [] },
      });
    ...
    const updated = await prisma.search.update({
      ...
      data: {
        ...
        queue: results,
        stats: { ...emptyStats(), discovered: results.length },
      },
    });
