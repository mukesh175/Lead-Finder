...
import { getBudget, callsFor } from "../search/quota";
...
async function planWithinBudget(requestedLimit) {
  ...
  const budget = await getBudget(provider.name);
  if (budget.remaining <= 0) {
    throw new ApiError(
      "SEARCH_FREE_TIER_REACHED",
      `Daily free-tier limit reached: all ${budget.limit} search API calls have been used today. ` +
        `The quota resets at ${budget.resetsAt}. No search was started and nothing was charged.`,
      429
    );
  }

  const needed = callsFor(requestedLimit);
  if (needed <= budget.remaining) return { resultLimit: requestedLimit, notice: null };

  const allowed = budget.resultsRemaining;
  return {
    resultLimit: allowed,
    notice:
      `Requested ${requestedLimit} leads, but only ${budget.remaining} free API call(s) remain today ` +
      `(daily limit ${budget.limit}) - this search was trimmed to ${allowed} results to stay inside the free tier.`,
  };
}
      ...
      data: {
        ...
        stats: {
          ...
          notice: results.truncatedByBudget
            ? `The daily free-tier budget ran out during this search - ${results.length} of ${resultLimit} ` +
              "requested results were retrieved. The quota resets at midnight Pacific Time."
            : notice,
        },
      },
    });
