...
export const mockProvider = {
  ...
  async search({ keyword, location, limit, includeDomains }) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      // Real searches return platform pages mixed in with company sites, so the
      // mock does too - that is what exercises the junk filtering.
      const platformProfile = !intentHost && n % 5 === 0;
      ...
      results.push({
        title: platformProfile ? "Instagram" : `${company} - ${titleCase(keyword)}`,
        url: intentHost
          ? `https://${intentHost}/post/${slug(keyword)}-${n % 9973}`
          : platformProfile
            ? `https://www.instagram.com/${slug(company)}/`
            : `https://${domain}/`,
        displayLink: domain,
        snippet: intentHost
        ...
        mockPages: platformProfile
          ? {
              [`https://www.instagram.com/${slug(company)}/`]:
                `<html><head><title>Instagram</title></head><body><h1>${company}</h1>` +
                `<p>Posted 18.08.2026 &middot; id 1787289066</p>` +
                `<a href="mailto:support-in@google.com">support</a></body></html>`,
            }
          : {
