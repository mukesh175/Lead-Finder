...
export const serperProvider = {
  ...
  async search({ keyword, location, limit, query, includeDomains }) {
    ...
    // Serper accepts Google operators, so domains become a site: OR group.
    const scoped =
      includeDomains && includeDomains.length > 0
        ? `${text} (${includeDomains.map((d) => `site:${d}`).join(" OR ")})`
        : text;
    ...
    for (let page = 1; results.length < wanted && page <= MAX_PAGES; page += 1) {
      ...
      try {
        response = await fetch(ENDPOINT, {
          ...
          body: JSON.stringify({ q: scoped, num: PAGE_SIZE, page }),
